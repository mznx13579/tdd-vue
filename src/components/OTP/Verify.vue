      ...
      <form class="login-form" @submit.prevent="verify">
        <div class="loginBox-ofc">
          ...
          <div class="body-ofc">
            ...
            <p class="id-regform" style="margin-bottom: 50px">모바일 기기를 소지하지 않았거나 분실하셨나요?</p>
          </div>
        </div>
      </form>
      <footer>
...
<script lang="ts">
import Vue from 'vue';
import { CustomVueRefs, StatusCode, ToastType } from '@/types/common';
import common from '@/common';
import authApi from '@/api/authApi';
import { OTPForm } from './types';

export default (Vue as CustomVueRefs<{ verifyCodeInput: HTMLInputElement }>).extend({
  name: 'Verify',
  ...
  mounted() {
    const verifyCodeInputElement = this.$refs.verifyCodeInput;
    verifyCodeInputElement.focus();
  },

  methods: {
    async verify() {
      if (!this.verifyPrechecker()) {
        return;
      }

      const verifyPayload: OTPForm = {
      ...
      try {
        ...
        const { data } = await authApi.create<OTPForm>('/otp/verify-code', verifyPayload);
